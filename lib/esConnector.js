  ...
  _.forEach(where, function (value, key) {
    ...
    if (key === 'and' && Array.isArray(value)) {
      ...
      if (spec) {
        ...
        if (spec === 'like') {
          var like = {};
          like.default_field="_all"
          like.query = cond
          /**
           * Apply the given filter in the main filter(body) and on given path
           */
          if (root) {
            path.bool.must.push({query_string : like});
          }
          else {
            path.query_string = like;
          }
        }
        /**
         * Logic for `exists` filter of where
         * @example {where: {role: {exists: false }}}
         */
        if (spec === 'exists') {
          var field = {exists : {field : key}}
          var condition = cond ? "must" : "must_not"
            if(root){
              path.bool[condition].push(field);
            }
            else {
              path.push({bool: {[condition]: [field]}});
            }
        }
